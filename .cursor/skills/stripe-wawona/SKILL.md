...
| Key | Where |
|---|---|
| Publishable `pk_test_` / `pk_live_` | Env `STRIPE_PUBLISHABLE_KEY` (SecretSpec/pass → Zola `get_env`). Never committed in `config.toml`. |
| Secret `sk_` / restricted `rk_` | `pass` → `secretspec/wawona/site/STRIPE_SECRET_KEY` or `secretspec/shared/default/STRIPE_SECRET_KEY` |
| Checkout API URL | Env `STRIPE_CHECKOUT_API` (local default `http://127.0.0.1:4242`) |

Never put `sk_` / `rk_` in `config.toml`, JS, HTML, GitHub Pages secrets,
...
Bootstrap: `pass-stripe-bootstrap` (stdin, no echo). See `wawona.io/docs/secrets.md`.
