...
**vercel:** OAuth refresh_token in pass → access token on each `vercel` invoke (cached ~8h). Wrapper calls `vercel-mint-token --write-auth` once (locked, curl/pass timeouts, atomic `auth.json` replace — never truncates on failure). Hot path is decrypt-free: cache validity uses the refresh `.gpg` mtime/size stamp (not `pass show`), so gpg/pinentry cannot hang every CLI invoke. Stale `~/.cache/dendritic/vercel-mint.lock` is reclaimed after 120s. Activation/wrapper probe secrets via `.gpg` existence only. If refresh returns HTTP 400, re-auth: `pass-vercel-bootstrap` or `vercel-mint-token --device`. Paths: Darwin `~/Library/Application Support/com.vercel.cli/auth.json`, Linux `~/.local/share/com.vercel.cli/auth.json`. Import: `pass-vercel-bootstrap --from-auth-json`. Static PAT: `--from-token TOKEN` / SecretSpec `VERCEL_TOKEN`. Optional team: `--team TEAM_ID`.
