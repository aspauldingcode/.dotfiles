  ...
  config = {
    ...
    system.activationScripts.writeEduroamConfig = lib.stringAfter ["var"] ''
      cat > /var/lib/iwd/eduroam.8021x <<EOF
        [Security]
        EAP-Method=PEAP
        EAP-Identity=$(cat ${config.sops.secrets.wifi_eduroam_userID.path})
        EAP-Password=$(cat ${config.sops.secrets.wifi_eduroam_passwd.path})
    ...
    system.activationScripts.writeBubblesPSK = lib.stringAfter ["var"] ''
      cat > /var/lib/iwd/Bubbles.psk <<EOF
        ...
        Passphrase=$(cat ${config.sops.secrets.wifi_bubbles_passwd.path})
