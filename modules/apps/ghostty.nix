{
  flake.modules.homeManager.dendritic =
    ...
    let
      ghosttyReloadCommand = pkgs.writeShellApplication {
        name = "ghostty-reload";
      ...
      config = lib.mkIf config.dendritic.apps.ghostty.enable {
        ...
        launchd.agents.ghostty-hot-reload = lib.mkIf pkgs.stdenv.isDarwin {
          ...
          config = {
            ...
            StandardErrorPath = "${config.home.homeDirectory}/.cache/ghostty-hot-reload.err.log";
          };
        };

        home.activation.syncGhosttyMacConfig = lib.mkIf pkgs.stdenv.isDarwin (
          lib.hm.dag.entryAfter [ "writeBoundary" ] ''
            app_support_dir="$HOME/Library/Application Support/com.mitchellh.ghostty"
            mkdir -p "$app_support_dir"
            ln -snf "$HOME/.config/ghostty/config" "$app_support_dir/config"
          ''
        );

        # Installs `ghostty-reload` so users can bind it in skhd/shkd/etc.
        home.packages = [
          config.programs.ghostty.package
          ghosttyReloadCommand
        ];
      };
    };

  # Dock registration: Ghostty owns its dock entry (order 140 in `dock.nix`).
  flake.modules.darwin.dendritic =
    ...
    {
      dendritic.dock.apps = lib.mkOrder 140 [
