{
  ...
  flake.modules.homeManager.dendritic =
    ...
    let
      ...
      # ── Database Resolution ──────────────────────────────────────
      rawImage =
        if builtins.hasAttr cfg.selected cfg.database then cfg.database."${cfg.selected}" else cfg.path;

      # ── Gowall theme name ────────────────────────────────────────
      # Defaults to the theme family configured in `theme-selection.nix`
      # (exposed as `dendritic.theme.name`). If gowall lacks a matching
      # theme, the user can override via `dendritic.wallpaper.gowall.theme`.
      gowallTheme = if cfg.gowall.theme != "" then cfg.gowall.theme else config.dendritic.theme.name;

      # ── Gowall Pipeline ──────────────────────────────────────────
      processedImage =
        if cfg.gowall.enable then
    ...
    in
    {
      ...
      config = lib.mkIf cfg.enable {
        ...
        home.activation.setWallpaper = lib.mkIf isDarwin (
