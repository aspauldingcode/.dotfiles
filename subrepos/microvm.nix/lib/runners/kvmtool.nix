...
in
{
  ...
  command =
    if user != null then
      ...
      throw "kvmtool does not support initialBalloonMem"
    else if hotplugMem != 0 then
      throw "kvmtool does not support hotplugMem"
    else if hotpluggedMem != 0 then
      throw "kvmtool does not support hotpluggedMem"
    else if credentialFiles != { } then
      throw "kvmtool does not support credentialFiles"
    else
      builtins.concatStringsSep " " (
        ...
        ++ builtins.concatMap (
          ...
          lib.warnIf (serial != null)
            ...
            [
              "-d"
              (lib.escapeShellArg "image${lib.optionalString direct ",direct"}${lib.optionalString readOnly ",ro"}")
            ]
        ) volumes
        ++ builtins.concatMap (
          {
