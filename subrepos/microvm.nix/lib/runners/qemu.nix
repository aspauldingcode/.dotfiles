...
let
  ...
  aioEngine = if vmHostPackages.stdenv.hostPlatform.isLinux then "io_uring" else "threads";
  ...
  accel =
    if vmHostPackages.stdenv.hostPlatform.isLinux then
      "kvm:tcg"
    else if vmHostPackages.stdenv.hostPlatform.isDarwin then
      "hvf:tcg"
    else
      "tcg";
  ...
  forwardingOptions = lib.concatMapStrings (
    {
      proto,
      from,
      host,
  ...
  kernelConsole =
    if !microvmConfig.qemu.serialConsole then
      ""
  ...
  systemdCredentialStrings = lib.mapAttrsToList (
    name: path: "name=opt/io.systemd.credentials/${name},file=${path}"
  ) credentialFiles;
...
lib.warnIf (mem == 2048)
  ...
  ''
  {
    ...
    command =
      if initialBalloonMem != 0 then
        throw "qemu does not support initialBalloonMem"
      else
        lib.escapeShellArgs (
          [
        ...
        + " "
        + lib.escapeShellArgs (
          builtins.concatMap (fwCfgOption: [
            "-fw_cfg"
            fwCfgOption
          ]) fwCfgOptions
          ...
          ++ lib.optionals storeOnDisk [
            "-drive"
            "id=store,format=raw,read-only=on,file=${storeDisk},if=none,aio=${aioEngine}"
            "-device"
          ...
          ++ lib.optionals balloon [
            ...
            (
              ...
              + lib.optionalString (deflateOnOOM) ",deflate-on-oom=on"
            )
          ]
          ++ lib.optionals useHotPlugMemory [
            "-object"
            "memory-backend-ram,id=vmem0,size=${toString hotplugMem}M"
            "-device"
            "virtio-mem-pci,id=vm0,memdev=vmem0,requested-size=${toString hotpluggedMem}M"
          ]
          ++ builtins.concatMap (
            {
              image,
            ...
            [
              ...
              "-device"
              "virtio-blk-${devType},drive=vd${letter}${lib.optionalString (serial != null) ",serial=${serial}"}"
            ]
          ) volumes
          ++ lib.optionals (shares != [ ]) (
            (lib.optionals vmHostPackages.stdenv.hostPlatform.isLinux [
              ...
              "node,memdev=mem"
              "-object"
              "memory-backend-memfd,id=mem,size=${toString mem}M,share=on"
            ])
            ++ builtins.concatMap (
              {
                proto,
              ...
              }:
              {
                ...
                "9p" = [
                  "-fsdev"
                  "local,id=fs${toString index},path=${source},security_model=${securityModel},readonly=${lib.boolToString readOnly}"
                  "-device"
                ...
                builtins.concatMap (
                  ...
                  [
                    ...
                    "virtio-net-${devType},netdev=${id},mac=${mac}${
                      ...
                      lib.optionalString (
                        requirePci || (microvmConfig.cpu == null && system != "x86_64-linux")
                      ) ",romfile="
