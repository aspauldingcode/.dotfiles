...
let
  ...
  devType =
    ...
    if requirePci then
      if addr < 32 then
        "pci,bus=pcie.0,addr=0x${lib.toHexString addr}"
      else
        throw "Too big PCI addr: ${lib.toHexString addr}"
    else
      "device";
...
in
{
  ...
  command =
    if balloon then
      throw "balloon not implemented for stratovirt"
    else if initialBalloonMem != 0 then
      throw "initialBalloonMem not implemented for stratovirt"
    else if hotplugMem != 0 then
      throw "stratovirt does not support hotplugMem"
    else if hotpluggedMem != 0 then
      throw "stratovirt does not support hotpluggedMem"
    else if credentialFiles != { } then
      throw "stratovirt does not support credentialFiles"
    else
      lib.escapeShellArgs (
        [
          ...
          initrdPath
          "-append"
          "console=${console} edd=off reboot=t panic=-1 ${toString microvmConfig.kernelParams}"
        ...
        ++ lib.optionals storeOnDisk [
          "-drive"
          "id=store,format=raw,readonly=on,file=${storeDisk},if=none,aio=io_uring,direct=false"
          "-device"
        ...
        ++ builtins.concatMap (
          ...
          [
            "-drive"
            "id=vd${letter},format=raw,if=none,aio=io_uring,file=${image},direct=${
              if direct then "on" else "off"
            },readonly=${if readOnly then "on" else "off"}"
            "-device"
            "virtio-blk-${devType (virtioblkOffset + index)},drive=vd${letter},id=blk_vd${letter}${
          ...
          builtins.concatMap (
            ...
            }:
            {
              "virtiofs" = [
                ...
                "-device"
                "vhost-user-fs-${
                  devType (virtiofsOffset + index)
                },chardev=fs${toString index},tag=${tag},id=fs${toString index}"
              ];
            }
            .${proto}
          ) (enumerate 0 shares)
