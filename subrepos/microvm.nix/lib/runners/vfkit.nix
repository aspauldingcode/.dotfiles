...
let
  ...
  ++ lib.optionals storeOnDisk [
    "--device"
    "virtio-blk,path=${storeDisk},readonly"
  ]
  ++ (builtins.concatMap (
    { image, ... }:
    [
      "--device"
      "virtio-blk,path=${image}"
    ]
  ) volumesWithLetters)
  ++ (builtins.concatMap (
    {
    ...
    if proto == "virtiofs" then
      [
        "--device"
        "virtio-fs,sharedDir=${source},mountTag=${tag}"
      ]
    else
  ...
  allArgs = [
    ...
    initrdPath
    "--kernel-cmdline"
    (builtins.concatStringsSep " " kernelCmdLine)
  ]
  ...
  command =
    let
      ...
      errors = [
        ...
        (check (
          vmHostPackages.stdenv.hostPlatform.isAarch64 != pkgs.stdenv.hostPlatform.isAarch64
    ...
    in
    if !valid then
      lib.findFirst (e: e != null) null errors
    else
      let
