...
let
  ...
  isRwStoreVirtiofsShare = builtins.any (
    { mountPoint, proto, ... }: mountPoint == config.microvm.writableStoreOverlay && proto == "virtiofs"
...
lib.mkIf config.microvm.guest.enable {
  ...
  systemd.mounts =
    lib.mkIf (config.boot.initrd.systemd.enable && !storeOnDisk && writableStoreOverlay == null)
      [
        {
          what = "store";
          where = "/nix/store";
